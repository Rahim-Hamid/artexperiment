...
def dither_effect(cv2_frame):
     ...
     dithered = pil_img.quantize(colors=4, method=2, dither=Image.FLOYDSTEINBERG)


     palette = dithered.getpalette()[:12]  
     arr = np.array(dithered)
     arr = np.array([palette[i*3] for i in arr.flatten()]).reshape(arr.shape)
